...
function Start(){
	...
	for(ground_count=0;ground_count< start_ground_amount; ground_count++){
		newgrounds = Instantiate(spawnees[0],Vector3((ground_count-1)*ground_length,0,0),Quaternion.identity);
	...
	if(playa.transform.position.x>player_start+ground_length)
	{
		...
		switch (segment_num)
		{
			...
			case 1:
				...
				newgrounds = Instantiate(spawnees[1],Vector3((ground_count++-1)*ground_length,3.92544,0),rotation);
				break;
			case 2:
				//spawn_prefab = Resources.Load("Terrain03") as Transform;
				newgrounds = Instantiate(spawnees[2],Vector3((ground_count++-1)*ground_length-4,8.5,0),Quaternion.identity);				
				break;
			default:
				break;
		}
